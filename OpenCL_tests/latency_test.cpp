...
const size_t MAX_STRIDE = 1024;

// OpenCL kernel to read from global memory with a stride
const char* kernelSource = R"(
__kernel void readGlobalMemory(__global float* input, __global float* output, const int stride) {
    int id = get_global_id(0) * stride;
    if (id < get_global_size(0) * stride) {
        output[id] = input[id];
    }
}
)";
...
int main() {
    ...
    // Measure latency for different strides
    for (size_t stride = 1; stride <= MAX_STRIDE; stride *= 2) {
        ...
        checkError(clSetKernelArg(kernel, 2, sizeof(int), &stride), "clSetKernelArg(2)");

        // Execute the kernel and measure the time
        auto start = std::chrono::high_resolution_clock::now();
        size_t globalWorkSize = DATA_SIZE / stride;
        checkError(clEnqueueNDRangeKernel(queue, kernel, 1, NULL, &globalWorkSize, NULL, 0, NULL, NULL), "clEnqueueNDRangeKernel");
        checkError(clFinish(queue), "clFinish");
        auto end = std::chrono::high_resolution_clock::now();
        std::chrono::duration<double> elapsedTime = end - start;

        // Write results to the file
        outputFile << stride << "," << elapsedTime.count() << "\n";
        std::cout << "Stride: " << stride << ", Latency: " << elapsedTime.count() << " seconds" << std::endl;
    }
